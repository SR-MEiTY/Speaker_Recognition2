...
def uploadVVAudio(file1, file2):
	...
	# 		print(isFound)
		...
		# file_name = "SSM32.wav"
		# full_file_name = os.path.join("/home/vocab/Speaker_Recognition/static/test_data/SSM32/", file_name)
		# file.save(full_file_name)

	# path2str  = "static/test_data/"
	path2str  = "static/test/"
	id="SSM32"
	sr=8000
	wavfilepath=path2str+id+ '/'+id+'.wav'
	...
	audiodata = AudioSegment.from_file(file1)
	...
	model.eval()
	with torch.no_grad():
		feat1 = model(d_c.reshape(bs * nxx,-1)).detach().cpu()
		feat1=feat1.reshape(bs,nxx,-1)
		#return feat
		#model_save = path2str+id+'/'+id+"_train.pth"
		#torch.save(feat, model_save)
	
	############# MODEL 2 ################
	
	audiodata = AudioSegment.from_file(file2)
	audiodata = audiodata.set_frame_rate(16000)
	audio = audiodata.get_array_of_samples()
	sample_rate = audiodata.frame_rate
	...
	model.eval()
	with torch.no_grad():
		feat2 = model(d_c.reshape(bs * nxx,-1)).detach().cpu()
		feat2=feat2.reshape(bs,nxx,-1)
		#return feat
		#model_save = path2str+id+'/'+id+"_test.pth"
		#torch.save(feat, model_save)
	...
	##############################################3
		...
		# test_feat=torch.load(path2str+id+'/'+id+"_train.pth")

		# speaker_feat=torch.load(path2enrol+id+'/'+id+"_train.pth")

		#speaker_feat=torch.load(path2enrol+id+'/'+id+"_train.pth")
		#test_feat=torch.load(path2str+id+'/'+id+"_test.pth")
	...
	score=cos(torch.mean(feat1, dim=1), torch.mean(feat2, dim=1))
	print(score)
	th = 0.93
	return score
...
if __name__ == '__main__':
    ...
    model.to(device="cpu")
    model.eval()
    f22=open("/home/vocab/Speaker_Recognition/static/trial_file","r")
    f22=f22.readlines()
    f22=[i.rstrip("\n") for i in f22]
    f33=open("/home/vocab/Speaker_Recognition/static/result_score","w")
    for nn in f22:
        k1=nn.split(" ")
        score=uploadVVAudio(k1[0],k1[1])
        f33.write("%s %s %s\n"%(k1[0],k1[1],str(score)))
